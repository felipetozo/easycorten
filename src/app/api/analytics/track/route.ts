...
function adSourceToSrc(adSource: string): string {
  switch (adSource) {
    case 'google':    return 'google_ads';
    case 'facebook':  return 'instagram_ads';
    case 'microsoft': return 'bing_ads';
    case 'tiktok':    return 'tiktok_ads';
    default: return adSource ? `${adSource}_ads` : '';
  }
}
...
const BOT_UA =
  /bot|crawl|spider|slurp|bingpreview|facebookexternalhit|facebot|ia_archiver|mediapartners|adsbot|pingdom|uptimerobot|statuscake|gtmetrix|lighthouse|headlesschrome|phantomjs|puppeteer|playwright|python-requests|axios|curl|wget|node-fetch|go-http|java\/|okhttp|semrush|ahrefs|mj12bot|dotbot|petalbot|yandex|baiduspider|duckduckbot|applebot/i;
...
type VisitorMeta = { ip?: string; ua?: string; language?: string; timezone?: string; screen?: string };
type FirstTouch  = { referrerSrc?: string; utmSource?: string; utmMedium?: string; utmCampaign?: string; adSource?: string };
...
async function upsertVisitor(id: string, meta: VisitorMeta = {}, first: FirstTouch = {}): Promise<void> {
...
export async function POST(request: NextRequest) {
  ...
  try {
    if (type === 'pageview') {
      ...
      const adClickId     = trunc(data.ad_click_id, 255);
      const adClickSource = trunc(data.ad_click_source, 40).toLowerCase();

      if (isAdminPath(path)) return NextResponse.json({ ok: true });

      const refSrc = classifySource(referrer, refDomain, utmSource, utmMedium, adClickSource, siteHost);

      await upsertVisitor(
        visitorId,
        { ip: clientIp(request), ua: userAgent, language: trunc(data.language, 20), timezone: trunc(data.timezone, 64), screen: trunc(data.screen, 20) },
        { referrerSrc: refSrc, utmSource, utmMedium, utmCampaign, adSource: adClickSource },
      );
      ...
      await prisma.analyticsPageview.create({
        data: {
          ...
          adClickSource:  adClickSource || null,
        },
      });
    } else if (type === 'time') {
      const path    = trunc(data.path, 500);
      ...
      if (seconds > 0) {
        const last = await prisma.analyticsPageview.findFirst({
          where: { sessionKey, path, visitorId, createdAt: { gt: new Date(Date.now() - 30 * 60 * 1000) } },
          orderBy: { createdAt: 'desc' },
          select: { id: true },
        });
        if (last) {
          await prisma.analyticsPageview.update({ where: { id: last.id }, data: { timeOnPage: seconds } });
        }
      }
    } else if (type === 'event') {
      ...
      await upsertVisitor(visitorId, { ip: clientIp(request), ua: request.headers.get('user-agent') ?? '' });

      await prisma.analyticsEvent.create({
        data: { visitorId, sessionKey: sessionKey || null, eventType, eventLabel: eventLabel || null, eventData, path: path || null },
      });
    }
